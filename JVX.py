...
from valuation_models import run_all_valuations, get_financial_data, safe_float
...
def processar_dados_acao(simbolo, hist, info, recom):
    ...
    # Coletar dados financeiros completos para os modelos de valuation
    financials_data = get_financial_data(simbolo)
    
    # Executar todos os modelos de valuation
    all_valuations = run_all_valuations(simbolo, info_processada, financials_data)

    # Para manter a compatibilidade com a estrutura existente, podemos pegar um 'preço justo' principal
    # ou usar a média dos modelos aplicáveis para o 'preco_justo' principal.
    # Por enquanto, vamos manter o cálculo original para 'preco_justo' e adicionar os novos modelos separadamente.
    valuation_antigo = calcular_preco_justo_yfinance(simbolo, info_processada, insights_simplificados)
    ...
    return {
        ...
        'valuation': valuation_antigo, # Mantém o valuation original para compatibilidade
        'all_valuations': all_valuations # Adiciona todos os novos valuations
    }
...
def atualizar_dados_completos_yfinance(acoes):
    ...
    for simbolo, dados_acao in resultados.items():
        ...
        resumo_list.append({
            ...
            'Todos os Valuations': dados_acao.get('all_valuations', {})
        })
    ...
    # Calcular Valuation Médio Ponderado
    df_resumo['Valuation Médio Ponderado'] = np.nan
    for index, row in df_resumo.iterrows():
        all_vals = row['Todos os Valuations']
        valid_vals = [v for v in all_vals.values() if pd.notna(v)]
        if valid_vals:
            df_resumo.loc[index, 'Valuation Médio Ponderado'] = np.mean(valid_vals)

    df_resumo_resumo = df_resumo.fillna({'Potencial': 0}) # Preenche potencial NaN com 0 para exibição
    return resultados, df_resumo_resumo
